...
2. Vendor an Android SDK (minimum platform 34) somewhere inside the repo (for example `tools/android-sdk/`) and add a checked-in
   `local.properties` that points `sdk.dir` to that relative path. Only the required pieces—`platforms/android-34/android.jar`,
   `build-tools/34.0.0`, and `platform-tools/`—are needed for unit tests.
