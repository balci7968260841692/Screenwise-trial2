...
## Checklist: reproduce the assistant's successful `assembleDebug`

Follow this exact sequence to mirror the environment where the build previously passed inside the sandbox:

1. **Pull the latest `please_work` branch** (or whichever branch already contains the dependency upgrades and Unicode fixes).
2. **Regenerate the Gradle wrapper JAR** on a machine with internet access:
   ```bash
   gradle wrapper --gradle-version 8.14.3 --distribution-type=bin
   ```
   Copy the resulting `gradle/wrapper/gradle-wrapper.jar` into the repo next to `gradle-wrapper.properties` (the `.gitignore`
   prevents it from being committed, but it must exist locally before running Gradle).
3. **Provide Gradle itself when operating offline.** Either rely on the regenerated wrapper (preferred) or drop an unpacked
   Gradle 8.14.3 distribution under `tools/gradle-8.14.3/` and invoke it via `tools/gradle-8.14.3/bin/gradle`.
4. **Bundle the Android 36 SDK + Build Tools 36.0.0.** Place the unpacked directories under `tools/android-sdk/` so the
   following paths exist:
   * `tools/android-sdk/platforms/android-36/`
   * `tools/android-sdk/build-tools/36.0.0/`
   * `tools/android-sdk/platform-tools/`
   A zipped SDK is fine if you also provide a script (for example `scripts/unpack-sdk.sh`) that extracts to those paths before
   the build runs.
5. **Create `local.properties` from the template.** Copy [`local.properties.example`](./local.properties.example) to
   `local.properties`, then set:
   ```properties
   ...
   ```
   Adjust `sdk.dir` if you stored the SDK elsewhere. Keep this file untracked.
6. **(Optional but recommended)** Deploy the Supabase SQL and Edge Functions if you want live AI responses. This step is not
   required to assemble the APK but ensures runtime parity with the production configuration.
7. **Run the build command:**
   ```bash
   ./gradlew clean assembleDebug
   ```
   If you are using a vendored Gradle distribution instead of the wrapper, execute
   `tools/gradle-8.14.3/bin/gradle --no-daemon --console=plain -Dorg.gradle.jvmargs="-Xmx4g" clean assembleDebug`.

With these assets and paths in place, the sandbox (and your local machine) will reproduce the previously successful
`assembleDebug` invocation without further tweaks.
